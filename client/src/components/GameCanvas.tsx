...
function drawBombs(ctx: CanvasRenderingContext2D, s: GameState) {
  for (const bomb of s.bombs) {
    ...
    const frac = 1 - bomb.timer / 120; // 0 → 1 as countdown progresses
...
function drawPlayer(
  ...
  player: { x: number; y: number; color: string; alive: boolean; dir: string; name: string; invincible: number; score: number },
  isMe: boolean
) {
...
// ─── OVERLAY PARTICLES (shimmer on floor) ─────────────────────────────────
